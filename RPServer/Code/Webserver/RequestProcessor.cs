...
namespace RemotePotatoServer
{
    class RequestProcessor : IDisposable
    {
        ...
        private string WebSvcCancelRecording(string ID)
        {
            if (!EPGManager.isWMCOnline)
                return "TV is not configured.";
        ...
        private string WebSvcRealCancelRecording64(string FilenameContainingTitle)
        {
            if (!EPGManager.isWMCOnline)
                return "TV is not configured.";
            ...
            //    {
            //        if (FilenameContainingTitle.ToLowerInvariant().IndexOf(item.GetExtendedProperty("Title").ToString().ToLowerInvariant()) != -1)
            //        {
            //            item.Cancel();
            //            return "Match!, so cancelling...";
            //            //break;
            //        }
            //    }
            //}
