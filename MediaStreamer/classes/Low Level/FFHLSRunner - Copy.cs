...
//                @"{THREADS} {H264PROFILE} {H264LEVEL} -flags +loop -g 30 -keyint_min 1 -bf 0 -b_strategy 0 -flags2 -wpred-dct8x8 -cmp +chroma -deblockalpha 0 -deblockbeta 0 -refs 1 {MOTIONSEARCHRANGE} {SUBQ} {PARTITIONS} -trellis 0 -coder 0 -sc_threshold 40 -i_qfactor 0.71 -qcomp 0.6 -qdiff 4 -rc_eq 'blurCplx^(1-qComp)' {MAPPINGS} {STARTTIME} {INPUTFILE} {ASPECT} {FRAMESIZE} {DEINTERLACE} -y {AUDIOSYNC} -f mpegts -vcodec libx264 {VIDEOBITRATE} {VIDEOBITRATEDEVIATION} -qmax 48 -qmin 2 -r 25 {AUDIOCODEC} {AUDIOBITRATE} {AUDIOSAMPLERATE} {AUDIOCHANNELS} {VOLUMELEVEL}" :                
                @"-re {THREADS} {H264PROFILE} {H264LEVEL} -flags +loop -g 30 -keyint_min 1 -bf 0 -b_strategy 0 -flags2 -wpred-dct8x8 -cmp +chroma -deblockalpha 0 -deblockbeta 0 -refs 1 {MOTIONSEARCHRANGE} {SUBQ} {PARTITIONS} -trellis 0 -coder 0 -sc_threshold 40 -i_qfactor 0.71 -qcomp 0.6 -qdiff 4 -rc_eq 'blurCplx^(1-qComp)' {MAPPINGS} {DURATION} {STARTTIME} {INPUTFILE} {ASPECT} {FRAMESIZE} {DEINTERLACE} -y {AUDIOSYNC} -f mpegts -vcodec libx264 {VIDEOBITRATE} {VIDEOBITRATEDEVIATION} -qmax 48 -qmin 2 -r 25 {AUDIOCODEC} {AUDIOBITRATE} {AUDIOSAMPLERATE} {AUDIOCHANNELS} {VOLUMELEVEL} " : 
 ...
 //           string strStartTime = "-ss %%i"; //when creating a batch file
            string strStartTime = "";
            ...
            // Duration in seconds to transcode (added by PihV), this - together with a loop in batch file - is to enable LiveTV for those users that cannot play past eof (from when the file is opened) of tv being recorded.
            int Duration = EncodingParameters.DurationBeforeMediaIsReopened;
            //string strDuration = (Duration <= 0) ? "" : ("-timelimit " + Duration.ToString());
            //string strDuration = (Duration <= 0) ? "" : ("-t " + Duration.ToString());
            string strDuration = (Duration <= 0) ? "" : ("");
            strFFMpegTemplate = strFFMpegTemplate.Replace("{DURATION}", strDuration);
...
#region HandleEOS
        public event EventHandler<GenericEventArgs<string>> HandleEOS;
        void shellRunner_HandleEOS(object sender, GenericEventArgs<string> e)
        {
            if (sender != shellRunner) return;
            if (e.Value != null) 
                HandleEOS (this, new GenericEventArgs<string>(e.Value));
        }
