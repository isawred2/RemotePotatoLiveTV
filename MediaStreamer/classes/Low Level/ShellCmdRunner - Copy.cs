...
namespace FatAttitude.MediaStreamer
{
    public class ShellCmdRunner
    {
        ...
        public int DurationBeforeMediaIsReopened { get; set; }
        ...
        public bool CreateBatchFile(string batchFilePath, int DurationBeforeMediaIsReopened)
        {
            ...
            //Changed by PihV
            //string strQuotedFileName = @"""" + shortFN + @"""";
            //            string strQuotedFileName = "for /L %%i in (0, " + DurationBeforeMediaIsReopened + ", 99999999999999999999) do " + @"""" + shortFN + @"""";
            string strQuotedFileName = "for /L %%i in (0, " + DurationBeforeMediaIsReopened + ", 99999999999999999999) do " + shortFN;
...
#if DEBUG
        const int ASSUME_FFMPEG_FINISHED_AFTER_TIMEOUT = 100;  // after 100 seconds of no output, we assume that we're hung on ReadBytes(1); above and at the end of the stream
#else
        ...
        void runningProcess_ErrorDataReceived(object sender, DataReceivedEventArgs e)
        {
            if (StandardErrorReceivedLine != null)
            {
                ...
                if (false) //(e.Data.Contains("Last message repeated"))
                {
                        if (!IsRunning) return;
                    WasKilledManually = true;
